...
import {
  setupBiome,
  setupTailwind,
} from "@/commands/start-new-project/partials/setup";
import type {
  Framework,
  FrameworkKey,
  FrameworkOptions,
...
const frameworks: Record<FrameworkKey, Framework> = {
  ...
  remix: {
    name: "Remix",
    createCommand: (packageManager, projectName) =>
      createRemixApp(packageManager, projectName),
  },
  vite: {
    name: "Vite",
    createCommand: (packageManager, projectName) =>
      createViteApp(packageManager, projectName),
  },
};
...
export async function startNewProject(
  ...
  if (["y", "yes"].includes(shouldStartNewProject.trim().toLowerCase())) {
    const filteredFrameworks =
      language === "javascript"
        ? Object.keys(frameworks).filter((key) =>
            ["laravel", "next"].includes(key)
          )
    ...
    if (framework === "next") {
      ...
      options.useSrc = ["y", "yes"].includes(
        wantSrcFolder.trim().toLowerCase()
      );
    }

    const useBiome = await input({
      message: `Do you want to use Biome for this project? (Y/${grayText(
        "n"
      )})`,
      default: "Yes",
      validate: (value) => {
        const normalizedValue = value.trim().toLowerCase();
        return (
          ["y", "n", "yes", "no"].includes(normalizedValue) ||
          "Please answer yes or no."
        );
      },
    });
    ...
    await executeCommand(
      startCreatingApp,
      `Creating ${frameworks[framework].name} project.`
    );
    ...
    if (shouldUseTailwindV4) {
      const upgradeTailwindCommand = [
        "npx",
        "@tailwindcss/upgrade@next",
        "--force",
      ];
      await executeCommand(
        upgradeTailwindCommand,
    ...
    // const cliCommand = isDev ? "intelligence-cli" : "intelligence-cli@latest"

    // const cliCommand = "intelligence-cli"
    const cliCommand = "intelligence-cli@latest";
    const tsOrJs = language === "typescript" ? "--ts" : "--js";
    const initJustdCommand = [
      "npx",
      cliCommand,
      "init",
      tsOrJs,
    ...
    if (framework === "laravel") {
      console.info(
        `Start your development server by running: ${highlight(
          `cd ${projectName} && composer run dev`
        )}\n`
      );
    } else {
      console.info(
    ...
    console.info(
      `Add new components by running: ${highlight(
        "npx intelligence-cli@latest add"
      )}`
    );
  } else {
    process.exit(0);
